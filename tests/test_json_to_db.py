...
def make_valid_variant(
    ...
    """Minimal valid ClinVar-like JSON entry."""
    return {
        "variant": variant,
        "g_hgvs": hgvs,
        "esummary": {
...
def mock_paths(monkeypatch, tmp_path):
    """Mock clinvar_folder and database_file paths."""
...
def mock_logger(monkeypatch):
    """Mock logger to capture all logs."""
    logs = []
    ...
    dummy = DummyLogger()
    monkeypatch.setattr(json_to_db, "logger", dummy)
    return logs
...
def test_no_json_files_logs_warning(mock_paths, mock_logger):
    ...
    assert any("No JSON files found" in msg for lvl, msg in mock_logger)
...
def test_json_load_failure_logs_exception(mock_paths, monkeypatch, mock_logger):
    ...
    json_to_db.json_to_dir()
    assert any("Failed to load JSON file" in msg for lvl, msg in mock_logger)
...
def test_skips_malformed_entry(mock_paths, mock_logger):
    ...
    json_to_db.json_to_dir()
    assert any("Skipping malformed entry" in msg for lvl, msg in mock_logger)
...
def test_missing_uid_skips_variant(mock_paths, mock_logger):
    ...
    json_to_db.json_to_dir()
    assert any("No ClinVar UID found" in msg for lvl, msg in mock_logger)
...
def test_uid_not_found_in_summary(mock_paths, mock_logger):
    ...
    json_to_db.json_to_dir()
    assert any("ClinVar UID 1 not found" in msg for lvl, msg in mock_logger)
...
def test_valid_variant_successful_insert(mock_paths, mock_inserts, mock_logger):
    ...
    assert any("Inserted variant" in msg for lvl, msg in mock_logger)
...
def test_invalid_allele_frequency_logs_warning(mock_paths, mock_inserts, mock_logger):
    ...
    json_to_db.json_to_dir()
    assert any("Invalid allele frequency" in msg for lvl, msg in mock_logger)
...
def test_database_insertion_failure_is_caught(mock_paths, monkeypatch, mock_logger):
    ...
    json_to_db.json_to_dir()
    assert any("Database insertion failed" in msg for lvl, msg in mock_logger)
